...
Plans and generates a multi-platform content calendar with post ideas, captions, hashtags, and optimal posting times. The user specifies duration, platforms, and themes — the AI builds a complete, actionable calendar. Output is saved as a structured markdown file that can be referenced daily for execution.
...
- **If the user does not specify a duration:** Default to 1 week (7 days) and inform: "I'm building a 1-week calendar. Want a longer period (2 weeks, 1 month)?"
...
- **If trend research via WebSearch returns no results:** Proceed without trend data and note: "I couldn't pull current trending data. The content uses evergreen best practices. You may want to check trending audio/formats on each platform before posting."
...
- **If the user provides a very high posting frequency (e.g., 5 posts/day across 5 platforms):** Flag the volume: "That's {N} total posts for the period. That's a significant content load. Want me to proceed, or scale back to a more sustainable cadence?"
