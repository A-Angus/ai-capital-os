...
Drafts personalized outreach to property sellers across every communication channel: physical letter, text message, voicemail drop script, email, and cold call script. Each piece is tailored to the seller's specific situation (pre-foreclosure, probate, absentee owner, tired landlord, divorce, code violations, tax delinquent, estate, etc.) with empathy, a clear value proposition, and a soft call to action. No sleazy "We Buy Houses" templates. These are crafted, situation-aware communications that build trust and get responses. Supports batch generation for multiple properties on a list.
...
```
~/Desktop/ai-os-blueprint/output/seller-outreach/{address-slug}/
...
```
~/Desktop/ai-os-blueprint/output/seller-outreach/batch-{date}/
├── 123-main-st/
├── 456-oak-ave/
└── 789-elm-blvd/
```
...
---
*Generated by AI OS Blueprint - Seller Outreach Drafter*
```
...
- **If the output directory does not exist:** Create `~/Desktop/ai-os-blueprint/output/seller-outreach/{address-slug}/` automatically before saving files.
...
- **If the user asks to actually send the messages (text, email):** Clarify: "I draft outreach — I don't send it. Copy the text messages into your phone, the emails into your email client, and print the letters. For batch texting, consider a platform like Launch Control or BatchLeads."
