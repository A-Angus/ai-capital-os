...
A file-based lead and contact pipeline manager that tracks prospects through defined sales stages. All data is stored as markdown files in `brain/pipeline/`, making it human-readable, version-controllable, and accessible to any AI session. Each lead gets its own file with full history. A master pipeline index provides the dashboard view.
